...
function Worker(config) {
    ...
    this.runServer(config.server.runServer);
}

Worker.prototype.runServer = function (config) {
    httpProxy.setMaxSockets(config.server.maxSockets);
    ...
    ipv4HttpServer.listen(config.server.port);
    ...
    ipv6HttpServer.listen(config.server.port, '::1');
    ...
    // If daemonized, demote thyself now that ports are open.
    if (config.server.daemon.enabled) {
      if (config.server.daemon.workerGroup !== 'root') {
        process.setgid(config.server.daemon.workerGroup);
      }
      if (config.server.daemon.workerUser !== 'root') {
        process.setuid(config.server.daemon.workerUser);
      }
    }
};
